...
//		updat.put(campos.get(""), event.getRowKey());
	...
	public static SQLiteDatabase getDBConnection() {
        ...
        File f = new File(App.getContext().getFilesDir()+"/"+DATABASE_NAME);
		SQLiteDatabase db=SQLiteDatabase.openDatabase(App.getContext().getFilesDir()+"/"+DATABASE_NAME,
    ...
    {   	
		ArrayList<String> nombre_subgrupo=new ArrayList<String>();
	 	...
	 	SQLiteDatabase db2= getDBConnection();
...
//    	    	 Log.e("Tema",c.getString(0)+"");
       	...
       	Log.e("TAMA�O",DomTema.size()+"");
    	if(DomTema.size()==0)
    	{
    		DomTema.add("null");
    		Log.e("TAMA�O-2",DomTema.size()+"");
    	}
    	return DomTema;
    }
    	...
    	{    		   
    		try
    		{
    	     do {   	    			    
    			...
    			Log.e("Error Li",e.toString());
			}	    		
    	}
	}
	
	public static void insertarDatosDB (EventoClassOldWEB event)
	{
		/***
		 * La idea es que cada evento de manera individual sera agregad
		 * Para tener en cuenta
